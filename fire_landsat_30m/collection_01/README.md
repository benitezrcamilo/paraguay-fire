...
1. Open the [MapBiomas Fire Landsat Burned Area Classification Notebook](https://colab.research.google.com/github/mapbiomas/paraguay-fire/blob/main/fire_landsat_30m/collection_01/mapbiomas_fire_classification_v1.ipynb) on Google Colab.
2. Clone this repository by running the following command in the notebook:
    ```bash
    !git clone https://github.com/mapbiomas/paraguay-fire.git
    ```
...
## Issues / TODO
- [ ] Validar se os passos de treinamento do modelo Deep Neural Network (DNN) estão adequados para as amostras e ecorregiões do Paraguai.
- [ ] Atualizar a seção "Repository Structure" para mapear os scripts reais usados na `collection_01` do MapBiomas Fire Paraguai.
